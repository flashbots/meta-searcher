#!/bin/sh
### BEGIN INIT INFO
# Provides:          searcher-ssh-key
# Required-Start:    $all
# Required-Stop:     
# Default-Start:     2 3 4 5
# Default-Stop:      0 1 6   
# Short-Description: Creates princess user and adds searcher ssh key
### END INIT INFO

case "$1" in
  start)
    echo "Starting searcher-ssh-key script"

    # Add princess user
    if ! id "princess" &>/dev/null; then
        adduser --disabled-password --gecos "" princess
        echo "Created princess user"
    else
        echo "Princess user already exists"
    fi

    # Create .ssh directory and set permission
    mkdir -p /home/princess/.ssh
    chmod 700 /home/princess/.ssh
    chown princess:princess /home/princess/.ssh

    # Add searcher's ssh key to authorized_keys file
    SSH_KEY="ssh-rsa  princess"
    echo "$SSH_KEY" > /home/princess/.ssh/authorized_keys
    chmod 600 /home/princess/.ssh/authorized_keys
    chown princess:princess /home/princess/.ssh/authorized_keys
    echo "Added Searcher SSH key to princess user's authorized_keys"

    # TEMP: Setting up local networking for runqemu with static IP: 192.168.7.2
    echo "TEMP: Setting up local networking for runqemu with static IP: 192.168.7.2"
    echo "nameserver 8.8.8.8" > /etc/resolv.conf
    cat <<EOF > /etc/network/interfaces
# /etc/network/interfaces -- configuration file for ifup(8), ifdown(8)
 
# The loopback interface
auto lo
iface lo inet loopback

# Wired or wireless interfaces including predictable names
auto eth0
iface eth0 inet static
	address 192.168.7.2
	netmask 255.255.255.0
	network 192.168.7.0
	gateway 192.168.7.1
EOF
    /etc/init.d/networking restart
    /etc/init.d/dropbear restart
    ;;
  stop)
    echo "Stopping searcher-ssh-key script"
    ;;
  *)
    echo "Usage: $0 {start|stop}"
    exit 1
    ;;
esac

exit 0